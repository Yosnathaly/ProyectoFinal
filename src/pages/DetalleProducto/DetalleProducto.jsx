...
const DetalleProducto = () => {
	...
	const producto = productos.find((productoFiltrada) => productoFiltrada.id == id);
	if(cargando) {
        return <div className="text-center mt-48 text-4xl font-medium">Cargando Características...</div>;
    }
	...
	return (
		<>
			<h2 className=" text-4xl text-center font-medium mt-24">Característica</h2>
			{!producto? (
				<p className="text-center bg-red-300 w-96 mx-auto text-red-700 mt-32 rounded-xl">Error, no tiene características...</p>
			) : (
				<div>
					<div className="container mx-auto  mt-12 py-3 flex flex-wrap justify-center">
						<img src={producto?.img} className=" border-slate-400 w-96 rounded-full" alt={`producto ${id}`} />
