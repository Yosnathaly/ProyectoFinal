...
const SeleccionaProducto = () => {
    ...
    const {id} = useParams()
	if(cargando) {
        return <div className="text-center mt-48 text-4xl font-medium">Mostrando productos...</div>;
    }
			...
			<div className=" container mx-auto gap-6 grid-productos text-center">
				{
					productos.map((producto) => (
						<div key={producto.id} className="border border-gray-300 rounded-full" style={{ width: "21rem" }}>
							<img className="rounded-full"src={producto.img} alt={`producto ${id}`} />
							<div className="p-3">
								<div>
									<h1 className=" text-2xl font-bold border-b-2">{producto.name.toUpperCase()}</h1>
								</div>
							...
							<p className="text-center text-2xl font-bold border-t-2 m-3"
								...
								<div className="flex justify-around card-body">
									<Link to={`producto/${producto.id}`} className="bg-ros border bg-yellow-400 border-neutral-500 px-4 py-2 rounded-xl">
										Caracteristicas
									</Link>
									<button onClick={() => handleAgregarAlCarro(producto)} className="px-4 py-2 bg-red-400 rounded-xl">
										Agregar al carro
									</button>
								</div>
							</div>							
